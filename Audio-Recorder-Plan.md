...
- ✅ React Native 0.81.5+
- ✅ Backward compatible with Bridge and forward compatible with TurboModules
...
## 4. New Architecture Support (TurboModules)

### Overview

The library supports **both old and new React Native architectures**:

- **Old Architecture**: Traditional Bridge (RCTBridgeModule)
- **New Architecture**: TurboModules + Fabric (JSI-based)

**Key Benefits:**

- ✅ Zero breaking changes for existing apps
- ✅ Opt-in new architecture support
- ✅ Better performance with new arch (direct JSI calls)
- ✅ Type-safe native-JS communication

### Architecture Detection

The library automatically detects which architecture is enabled at runtime:
...
const AudioRecorderModule =
  TurboModuleRegistry.get<Spec>('AudioRecorder') ?? // New Arch
  NativeModules.AudioRecorder; // Old Arch
...
Create `src/NativeAudioRecorder.ts` for codegen:

```typescript
// src/NativeAudioRecorder.ts
...
Update `AudioRecorder.mm` (Objective-C++ bridge):

```objc
// ios/AudioRecorder.mm
#import <React/RCTBridgeModule.h>
#import <React/RCTEventEmitter.h>

#ifdef RCT_NEW_ARCH_ENABLED
#import "AudioRecorderSpec.h"
#endif
...
Update Swift implementation to conform to generated protocol (when new arch):
...
class AudioRecorder: RCTEventEmitter {
  ...
  // All @objc methods work for both old and new arch
}

#if RCT_NEW_ARCH_ENABLED
// Conform to TurboModule protocol
extension AudioRecorder: NativeAudioRecorderSpec {
  // Protocol conformance is automatic due to @objc methods
}
#endif
```
...
Update `AudioRecorderModule.kt` for new architecture:

```kotlin
// android/src/main/java/com/choiminseok/audiorecorder/AudioRecorderModule.kt
...
class AudioRecorderModule(reactContext: ReactApplicationContext) :
    ...
    TurboModule { // Implement TurboModule interface
    ...
    // All existing @ReactMethod implementations remain the same
    // They work for both old and new architecture
...
Create `AudioRecorderTurboModuleSpec.kt` for explicit TurboModule interface (optional, for type safety):

```kotlin
// android/src/main/java/com/choiminseok/audiorecorder/AudioRecorderTurboModuleSpec.kt
package com.choiminseok.audiorecorder
...
abstract class AudioRecorderTurboModuleSpec(reactContext: ReactApplicationContext) :
    ReactContextBaseJavaModule(reactContext),
    TurboModule {

    abstract fun checkPermission(promise: Promise)
    abstract fun requestPermission(promise: Promise)
    abstract fun startRecording(config: ReadableMap, promise: Promise)
    abstract fun pauseRecording(promise: Promise)
    abstract fun resumeRecording(promise: Promise)
    abstract fun stopRecording(promise: Promise)
    abstract fun cancelRecording(promise: Promise)
    abstract fun getState(promise: Promise)
    abstract fun getDuration(promise: Promise)
}
```

### Codegen Configuration

The `codegenConfig` in package.json triggers automatic code generation:
...
- iOS: `AudioRecorderSpec.h` / `AudioRecorderSpec-generated.mm`
- Android: `NativeAudioRecorderSpec.java`

### Build Configuration

**iOS Podspec** (`AudioRecorder.podspec`):
...
Pod::Spec.new do |s|
  ...
  s.pod_target_xcconfig = {
    "DEFINES_MODULE" => "YES",
    "SWIFT_OBJC_INTERFACE_HEADER_NAME" => "AudioRecorder-Swift.h",
    "SWIFT_VERSION" => "5.0"
  }
...
**Android CMakeLists.txt** (if using C++ - optional):
...
# Source files (if you add C++ implementation)
add_library(
  audiorecorder
  SHARED
  # cpp/AudioRecorderModule.cpp (optional)
  # For now, we're using pure Kotlin, so this is optional
)

target_include_directories(audiorecorder PRIVATE .)
...
## 5. iOS Native Implementation (Swift)
